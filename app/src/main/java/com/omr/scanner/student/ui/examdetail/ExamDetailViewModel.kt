package com.omr.scanner.student.ui.examdetail

import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.google.gson.Gson
import com.omr.scanner.student.data.model.Exam
import com.omr.scanner.student.data.model.ExamSubmissionItem
...
class ExamDetailViewModel(
    ...
    private fun bindExam(exam: MobileExamDetailResponse) {
        ...
        _examType.value = exam.examType
        _gradingType.value = "STUDENT_SUBMISSION"
        _templateName.value = exam.template?.name ?: "Mẫu OMR"
        _progressText.value = "San sang nop bai"
        ...
        viewModelScope.launch {
            appealsRepository.getAppeals(status = "PENDING", examId = examId).collect { result ->
                if (result is ApiResult.Success) {
                    _pendingAppealCount.value = result.data.meta.total
                }
            }
        }
    }
